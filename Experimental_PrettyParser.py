...
stick2def = raw_input("Press Enter for Yaas or 'N' for Nooo: ").lower()
...
if stick2def == 'n':
    ...
    while True:
        link = raw_input("Input RSS URL: ")
        if link == '':
            break
        tag = raw_input("Input a name tag for this feed: ")
        feeds[tag] = link
...
#            resp = requests.get(url_string)
            resp = http.request('GET',url_string)
...
def user_queries():
    ...
    no_rep = True
    if raw_input("Do you want the articles to repeat after you've viewed them once? (Y/N) ").lower() == "Y":
        no_rep = False
    num = raw_input("How many articles do you want to see per page? Press Enter to use default (25): ")
    if num == '':
        num = 25
    num = int(num)
...
#    text_paras = article.text.split('\n')
    ...
    '''
#    article_html_file = open(str('tmp_article_'+str(counter)+'.html'), 'w+')
...
#        article = Article(article_url)
    ...
    again = raw_input("More articles? Press Enter for Hell Yeah or any other key for Nuh-uh. : ").lower()
    if again == '':
